      ...
      <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-gray-50 to-white overflow-hidden shadow-[inset_-10px_-10px_30px_rgba(0,0,0,0.03),_0_20px_40px_rgba(0,0,0,0.05)] border border-gray-100/50 flex items-center justify-center">
        ...
        >
          ...
          <Geographies geography={geoUrl}>
            {({ geographies }) =>
              (geographies as SimpleGeography[]).map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
                  fill="#bfdbfe"
          ...
          >
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <input
                ...
                className={`w-full px-4 py-2 text-sm text-black border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${error ? "border-red-500 bg-red-50" : "border-gray-200"
                  }`}
                autoFocus
              />
              <button
