...
export default function ThinkPageError({ error, reset }: { error: Error; reset: () => void }) {
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6">
      <div className="w-16 h-16 bg-red-50 rounded-3xl flex items-center justify-center mx-auto mb-6">
        <span className="text-2xl">⚠️</span>
      </div>
      <p className="text-zinc-900 font-bold text-lg mb-2">Errore di caricamento</p>
      <p className="text-zinc-400 text-sm mb-6 text-center max-w-sm">
        Il pensiero non è stato caricato. Potrebbe essere stato rimosso o esserci un problema di connessione.
      </p>
      <button
        onClick={reset}
