...
DROP TABLE IF EXISTS public.translations_cache CASCADE;

-- Recreate translations_cache with BIGINT for entity_id to match chats.id / risposte.id
CREATE TABLE IF NOT EXISTS public.translations_cache (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    entity_id BIGINT NOT NULL, -- Either chat_id or risposta_id
    entity_type TEXT NOT NULL CHECK (entity_type IN ('chat', 'reply')),
    target_lang TEXT NOT NULL,
    original_lang TEXT,
    translated_title TEXT, -- For chats
    translated_text TEXT NOT NULL, -- For both (chat.titolo or risposta.testo)
...
-- Allow authenticated users to insert new translations, and anon users if needed
CREATE POLICY "Translations can be created by everyone" ON public.translations_cache
    FOR INSERT WITH CHECK (true);

-- Update RPC for global search across chats and replies to fix column reference
